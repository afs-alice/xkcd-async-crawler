...
class XkcdAsyncDownloader:
    ...
    async def __task_of_downloader(self, comic_id: int, session: aiohttp.client.ClientSession) -> None:
        ...
        if not image_file_data:
            return
        if not await self.__save_file_in_local_storage(comic_id, image_file_data['content'],
    ...
    async def __get_image_file(self, comic_id: int, image_url: str,
        ...
        if captured_except():
            logging.warning((
                f'Error {type(captured_except()).__name__} in request for comic id image file: {comic_id}')
            )
            return False
    ...
    async def __get_image_comic_url(self, comic_id: int,
        ...
        if captured_except():
            logging.warning(f'Error {type(captured_except()).__name__} in request for comic '
                            'id image file: {comic_id}')
    ...
    async def __get_last_index(self, session: aiohttp.client.ClientSession) -> Union[bool, int]:
        ...
        if captured_except():
            logging.error(
                f'Error {type(captured_except().__name__)} in request last comic index from xkcd API'
            )
    ...
    async def __save_file_in_local_storage(self, comic_id: int, file_content: bytes,
        ...
        if captured_except():
            logging.error(
                f'Error {type(captured_except()).__name__} when save file image for '
    ...
    @contextmanager
    def __get_except(self) -> Union[None, Exception]:
        is_except = None
        try:
